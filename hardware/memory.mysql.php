    ...
    if (check_userlevel($db, $AL_Edit)) {
      ...
      if (mysqli_num_rows($q_inv_models) > 0) {
        while ($a_inv_models = mysqli_fetch_array($q_inv_models)) {
          ...
          $linkstart = "<a href=\"#\" onclick=\"show_file('memory.fill.php?id="  . $a_inv_models['mod_id'] . "');jQuery('#dialogUpdate').dialog('open');return false;\">";
          $linkdel   = "<input type=\"button\" value=\"Remove\"  onclick=\"delete_line('memory.del.php?id=" . $a_inv_models['mod_id'] . "');\">";
          $linkend   = "</a>";
