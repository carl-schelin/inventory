    ...
    if (check_userlevel($db, $AL_Edit)) {
      ...
      if (mysqli_num_rows($q_models) > 0) {
        while ($a_models = mysqli_fetch_array($q_models)) {
          ...
          $linkstart = "<a href=\"#\" onclick=\"show_file('storage.fill.php?id="  . $a_models['mod_id'] . "');jQuery('#dialogUpdate').dialog('open');return false;\">";
          $linkdel   = "<input type=\"button\" value=\"Remove\"  onclick=\"delete_line('storage.del.php?id=" . $a_models['mod_id'] . "');\">";
          $linkend   = "</a>";
          ...
          $output .= "  <td class=\"" . $class . " delete\">"              . $a_models['mod_speed']           . "</td>";
        ...
        $output .= "<tr>";
        $output .= "  <td class=\"ui-widget-content\" colspan=\"6\">No storage devices found</td>";
        $output .= "</tr>";
      }
