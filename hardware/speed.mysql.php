    ...
    if (check_userlevel($db, $AL_Edit)) {
      if ($formVars['update'] == 0 || $formVars['update'] == 1) {
        ...
        if (strlen($formVars['spd_text']) > 0) {
          ...
          if ($formVars['update'] == 0) {
            $q_string = "insert into inv_int_speed set spd_id = NULL, " . $q_string;
          }
          if ($formVars['update'] == 1) {
            $q_string = "update inv_int_speed set " . $q_string . " where spd_id = " . $formVars['id'];
          }
      ...
      if (mysqli_num_rows($q_inv_int_speed) > 0) {
        while ($a_inv_int_speed = mysqli_fetch_array($q_inv_int_speed)) {
          ...
          $linkstart = "<a href=\"#\" onclick=\"show_file('speed.fill.php?id=" . $a_inv_int_speed['spd_id'] . "');jQuery('#dialogUpdate').dialog('open');return false;\">";
          $linkdel   = "<input type=\"button\" value=\"Remove\" onclick=\"delete_line('speed.del.php?id=" . $a_inv_int_speed['spd_id'] . "');\">";
          $linkend   = "</a>";

          $total = 0;
          $q_string  = "select int_id ";
          $q_string .= "from interface ";
          $q_string .= "where int_speed = " . $a_inv_int_speed['spd_id'] . " ";
          ...
          $output .= "  <td class=\"" . $class . "\">"        . $linkstart . $a_inv_int_speed['spd_text'] . $linkend . "</td>";
