      ...
      if ($formVars['update'] == 0 || $formVars['update'] == 1) {
        ...
        if (strlen($formVars['lic_vendor']) > 0) {
          ...
          if ($formVars['update'] == 0) {
            $q_string = "insert into inv_licenses set lic_id = NULL, " . $q_string;
          }
          if ($formVars['update'] == 1) {
            $q_string = "update inv_licenses set " . $q_string . " where lic_id = " . $formVars['id'];
          }
...
##############
      ...
      $q_string .= "left join inv_products on inv_products.prod_id = inv_licenses.lic_project ";
      $q_string .= "left join inv_vendors  on inv_vendors.ven_id   = inv_licenses.lic_vendor ";
      $q_string .= $orderby;
      $q_inv_licenses = mysqli_query($db, $q_string) or die ($q_string . ": " . mysqli_error($db));
      if (mysqli_num_rows($q_inv_licenses) > 0) {
        while ($a_inv_licenses = mysqli_fetch_array($q_inv_licenses)) {

          $linkstart = "<a href=\"#\" onclick=\"show_file('license.fill.php?id="  . $a_inv_licenses['lic_id'] . "');jQuery('#dialogUpdate').dialog('open');return false;\">";
          $linkdel   = "<input type=\"button\" value=\"Remove\" onclick=\"delete_line('license.del.php?id=" . $a_inv_licenses['lic_id'] . "');\">";
          $linkend   = "</a>";
          ...
          $output .= "<td class=\"ui-widget-content\">" . $linkstart . $a_inv_licenses['ven_name']     . $linkend . "</td>\n";
          $output .= "<td class=\"ui-widget-content\">"              . $a_inv_licenses['lic_product']             . "</td>\n";
          $output .= "<td class=\"ui-widget-content\">"              . $a_inv_licenses['lic_date']                . "</td>\n";
          $output .= "<td class=\"ui-widget-content\">"              . $a_inv_licenses['lic_vendorpo']            . "</td>\n";
          $output .= "<td class=\"ui-widget-content\">"              . $a_inv_licenses['lic_po']                  . "</td>\n";
          $output .= "<td class=\"ui-widget-content\">"              . $a_inv_licenses['prod_name']               . "</td>\n";
          $output .= "<td class=\"ui-widget-content\">"              . $a_inv_licenses['lic_quantity']            . "</td>\n";
          $output .= "<td class=\"ui-widget-content\">"              . $a_inv_licenses['lic_key']                 . "</td>\n";
          $output .= "<td class=\"ui-widget-content\">"              . $a_inv_licenses['lic_serial']              . "</td>\n";
          $output .= "<td class=\"ui-widget-content\">"              . $a_inv_licenses['lic_domain']              . "</td>\n";
          $output .= "</tr>\n";
        }
      } else {
        $output .= "<tr>\n";
