...
# activate the entry for this item
  ...
  while ($a_inv_svr_software = mysqli_fetch_array($q_inv_svr_software)) {
    $invindex[$a_inv_svr_software['svr_companyid']] = true;
  }
...
# wanted to do a dns lookup but it was taking forever so disable for now
  ...
  while ($a_inv_inventory = mysqli_fetch_array($q_inv_inventory)) {
    ...
    if ($invindex[$a_inv_inventory['inv_id']] === true) {
      ...
      $q_string = "select grp_name "
                . "from inv_groups "
                . "where grp_id = " . $a_inv_inventory['inv_appadmin'] . " ";
      $q_inv_groups = mysqli_query($db, $q_string) or die(header("Location: " . $Siteroot . "/error.php?script=" . $package . "&error=" . $q_string . "&mysql=" . mysqli_error($db)));
      $a_inv_groups = mysqli_fetch_array($q_inv_groups);
      ...
      $q_string .= "where int_companyid = \"" . $a_inv_inventory['inv_id'] . "\" and int_type != 7 and int_addr != '' and int_ip6 = 0 ";
...
# if a console or LOM interface type
      ...
      if (check_userlevel($db, $AL_Edit)) {
        ...
        if (check_grouplevel($db, $a_inv_inventory['inv_manager'])) {
          ...
          $edipstart = "<a href=\"" . $Editroot . "/inventory.php?server=" . $a_inv_inventory['inv_id'] . "#network\"  target=\"_blank\">" . $editpencil;
...
# used only for the server to view the inventory data;
      ...
      $shipstart    = "<a href=\"" . $Showroot . "/inventory.php?server="  . $a_inv_inventory['inv_id'] . "#network\"  target=\"_blank\">";
      $showend = "</a>";
...
# print the actual data row
      if ($formVars['csv'] == 'false') {
        ...
        print "  <td " . $class . "><nobr>" . $a_inv_inventory['loc_name']  . $showend . "</nobr></td>\n";
        print "  <td " . $class . "><nobr>" . $a_inv_inventory['loc_addr1'] . $showend . "</nobr></td>\n";
        print "  <td " . $class . "><nobr>" . $a_inv_inventory['loc_addr2'] . $showend . "</nobr></td>\n";
        print "  <td " . $class . "><nobr>" . $a_inv_inventory['loc_suite'] . $showend . "</nobr></td>\n";
        print "  <td " . $class . ">" . $a_inv_inventory['ct_city']         . $showend . "</td>\n";
        print "  <td " . $class . ">" . $a_inv_inventory['st_acronym']      . $showend . "</td>\n";
        print "  <td " . $class . ">" . $a_inv_inventory['loc_zipcode']     . $showend . "</td>\n";
        print "  <td " . $class . ">" . $a_inv_inventory['cn_acronym']      . $showend . "</td>\n";
        print "  <td " . $class . ">" . $a_inv_inventory['loc_identity']    . $showend . "</td>\n";
        print "</tr>\n";
      } else {
        ...
        print "\"" . $a_inv_inventory['loc_addr1'] . "\",";
        print "\"" . $a_inv_inventory['loc_addr2'] . "\",";
        print "\"" . $a_inv_inventory['loc_suite'] . "\",";
        print "\"" . $a_inv_inventory['ct_city'] . "\",";
        print "\"" . $a_inv_inventory['st_acronym'] . "\",";
        print "\"" . $a_inv_inventory['loc_zipcode'] . "\",";
        print "\"" . $a_inv_inventory['cn_acronym'] . "\",";
