  ...
  while ($a_inventory = mysql_fetch_array($q_inventory)) {
    ...
    if (check_userlevel($AL_Edit)) {
      if ($a_inventory['inv_manager'] == $_SESSION['group']) {
        $linkedit = "<a href=\"" . $Editroot . "/inventory.php?server=" . $a_inventory['inv_id'] . "\" target=\"_blank\"><img src=\"/inventory/imgs/pencil.gif\" height=10></a>";
      }
    }
    ...
    $q_child = mysql_query($q_string) or die($q_string . ": " . mysql_error());
    while ($a_child = mysql_fetch_array($q_child)) {

      $linkedit = '';
      if (check_userlevel($AL_Edit)) {
        if ($a_child['inv_manager'] == $_SESSION['group']) {
          $linkedit = "<a href=\"" . $Editroot . "/inventory.php?server=" . $a_child['inv_id'] . "\" target=\"_blank\"><img src=\"/inventory/imgs/pencil.gif\" height=10></a>";
        }
      }
