...
<title>Openview Policy Report</title>
...
# or country == 0 and location >  0
  ...
  $passthrough = "&group=" . $formVars['group'] . "&product=" . $formVars['product'] . "&project=" . $formVars['project'] . "&inwork=" . $formVars['inwork'];

  print "<table class=\"ui-styled-table\">\n";
  print "<tr>\n";
  print "  <th class=\"ui-state-default\">Openview Policy Table</th>\n";
  ...
  print "<p>The purpose of this report is to display all the Openview policies based on the Filter criteria.</p>\n";
  ...
  print "  <th class=\"ui-state-default\"><a href=\"" . $package . "?sort=pt_type"  . $passthrough . "\">Type</a></th>\n";
  print "  <th class=\"ui-state-default\"><a href=\"" . $package . "?sort=pd_description"   . $passthrough . "\">Description</a></th>\n";
  print "  <th class=\"ui-state-default\"><a href=\"" . $package . "?sort=pol_status" . $passthrough . "\">Status</a></th>\n";
  print "  <th class=\"ui-state-default\"><a href=\"" . $package . "?sort=pol_version"   . $passthrough . "\">Version</a></th>\n";
  print "  <th class=\"ui-state-default\"><a href=\"" . $package . "?sort=pol_date"    . $passthrough . "\">Updated</a></th>\n";
  print "</tr>\n";

  $q_string  = "select pol_id,inv_id,inv_name,pt_type,pd_description,pol_status,pol_version,pol_date ";
  $q_string .= "from policy ";
  $q_string .= "left join inventory on inventory.inv_id = policy.pol_companyid ";
  ...
  $q_string .= $where;
  $q_string .= $orderby;
  $q_policy = mysqli_query($db, $q_string) or die(header("Location: " . $Siteroot . "/error.php?script=" . $package . "&error=" . $q_string . "&mysql=" . mysqli_error($db)));
  if (mysqli_num_rows($q_policy) == 0) {
    print "<tr>";
    print "  <td" . $class . ">" . $a_policy['inv_name']       . "</td>";
    print "  <td" . $class . ">" . "No policies found" . "</td>";
    print "</tr>";
  } else {
    while ($a_policy = mysqli_fetch_array($q_policy)) {
      ...
      print "<tr>";
      print "  <td" . $class . ">" . $a_policy['inv_name']       . "</td>";
      print "  <td" . $class . ">" . $a_policy['pt_type']        . "</td>";
      print "  <td" . $class . ">" . $a_policy['pd_description'] . "</td>";
      print "  <td" . $class . ">" . $status                     . "</td>";
      print "  <td" . $class . ">" . $a_policy['pol_version']    . "</td>";
      print "  <td" . $class . ">" . $a_policy['pol_date']       . "</td>";
      print "</tr>";

    }
  }

  mysqli_free_result($q_policy);
?>
</table>
</div>
