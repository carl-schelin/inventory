...
# Description: Fill in the table for editing.
  ...
  if (isset($_SESSION['username'])) {
    ...
    if (check_userlevel($db, $AL_Edit)) {
      ...
      $group = return_Index($db, $a_inv_filesystem['fs_group'], "select grp_id from inv_groups where grp_disabled = 0 order by grp_name");
      ...
      print "document.formFilesystemUpdate.fs_lun.value = '"       . mysqli_real_escape_string($db, $a_inv_filesystem['fs_lun'])       . "';\n";
      print "document.formFilesystemUpdate.fs_volid.value = '"     . mysqli_real_escape_string($db, $a_inv_filesystem['fs_volid'])     . "';\n";
      print "document.formFilesystemUpdate.fs_path.value = '"      . mysqli_real_escape_string($db, $a_inv_filesystem['fs_path'])      . "';\n";

      if ($group > 0) {
        print "document.formFilesystemUpdate.fs_group['" . $group . "'].selected = true;\n";
      }

      if ($a_inv_filesystem['fs_backup']) {
