...
# Description: Fill in the table for editing.
  ...
  if (isset($_SESSION['username'])) {
    ...
    if (check_userlevel($db, $AL_Edit)) {
      logaccess($db, $_SESSION['uid'], $package, "Requesting record " . $formVars['id'] . " from inv_hardware");
...
// retrieve type list
      ...
      $q_string .= "where mod_type = " . $a_inv_hardware['hw_type'] . " ";
...
# create the javascript bit for populating the model dropdown box.
      ...
      $product  = return_Index($db, $a_inv_hardware['hw_product'],   "select prod_id from inv_products order by prod_name");
      $model    = return_Index($db, $a_inv_hardware['hw_vendorid'],  "select mod_id from inv_models left join inv_vendors on inv_vendors.ven_id = inv_models.mod_vendor where mod_type = " . $a_inv_hardware['hw_type'] . " order by ven_name,mod_name");
      $type     = return_Index($db, $a_inv_hardware['hw_type'],      "select part_id from inv_parts order by part_name");
      $support  = return_Index($db, $a_inv_hardware['hw_supportid'], "select sup_id from inv_support order by sup_company,sup_contract");
      $response = return_Index($db, $a_inv_hardware['hw_response'],  "select slv_id from inv_supportlevel order by slv_value");
      $hwselect = return_Index($db, $a_inv_hardware['hw_hw_id'],     "select hw_id from inv_hardware where hw_companyid = " . $a_inv_hardware['hw_companyid'] . " and hw_hw_id = 0 and hw_id != " . $formVars['id']);
      $hwdisk   = return_Index($db, $a_inv_hardware['hw_hd_id'],     "select hw_id from inv_hardware left join inv_models on inv_models.mod_id = inv_hardware.hw_vendorid where hw_companyid = " . $a_inv_hardware['hw_companyid'] . " and mod_name like \"RAID%\" and hw_id != " . $formVars['id']);

      print "document.edit.hw_serial.value = '"    . mysqli_real_escape_string($db, $a_inv_hardware['hw_serial'])    . "';\n";
      print "document.edit.hw_asset.value = '"     . mysqli_real_escape_string($db, $a_inv_hardware['hw_asset'])     . "';\n";
      print "document.edit.hw_purchased.value = '" . mysqli_real_escape_string($db, $a_inv_hardware['hw_purchased']) . "';\n";
      print "document.edit.hw_built.value = '"     . mysqli_real_escape_string($db, $a_inv_hardware['hw_built'])     . "';\n";
      print "document.edit.hw_active.value = '"    . mysqli_real_escape_string($db, $a_inv_hardware['hw_active'])    . "';\n";
      print "document.edit.hw_retired.value = '"   . mysqli_real_escape_string($db, $a_inv_hardware['hw_retired'])   . "';\n";
      print "document.edit.hw_reused.value = '"    . mysqli_real_escape_string($db, $a_inv_hardware['hw_reused'])    . "';\n";
      print "document.edit.hw_eolticket.value = '" . mysqli_real_escape_string($db, $a_inv_hardware['hw_eolticket']) . "';\n";
      print "document.edit.hw_rma.value = '"       . mysqli_real_escape_string($db, $a_inv_hardware['hw_rma'])       . "';\n";
      print "document.edit.hw_note.value = '"      . mysqli_real_escape_string($db, $a_inv_hardware['hw_note'])      . "';\n";
      ...
      if ($a_inv_hardware['hw_supid_verified']) {
