  ...
  if (isset($_SESSION['username'])) {
    ...
    if (check_userlevel($db, $AL_Edit)) {
      ...
      if (mysqli_num_rows($q_inventory) > 0) {
        ...
        $invresponse   = return_Index($db, $a_inventory['inv_response'],   "select slv_id  from inv_supportlevel order by slv_value");
        $invproduct    = return_Index($db, $a_inventory['inv_product'],    "select prod_id from inv_products order by prod_name");
        $invproject    = return_Index($db, $a_inventory['inv_project'],    "select prj_id  from inv_projects where prj_product = " . $a_inventory['inv_product'] . " order by prj_name");
        ...
        $invenv        = return_Index($db, $a_inventory['inv_env'],        "select env_id  from inv_environment order by env_name");
