...
# Description: Marking a hardware record as deleted
  ...
  if (isset($_SESSION['username'])) {
    ...
    if (check_userlevel($db, $AL_Edit)) {
      logaccess($db, $_SESSION['uid'], $package, "Marking Record " . $formVars['id'] . " as deleted from inv_hardware");
...
# get the inv_id
      ...
      $q_string .= "from inv_hardware ";
      $q_string .= "where hw_companyid = " . $a_inv_hardware['hw_companyid'] . " and hw_id != " . $formVars['id'] . " and hw_primary = 1 ";
      $q_inv_hardware = mysqli_query($db, $q_string) or die(header("Location: " . $Siteroot . "/error.php?script=" . $package . "&error=" . $q_string . "&mysql=" . mysqli_error($db)));
      if (mysqli_num_rows($q_inv_hardware) > 0) {
        $q_string  = "update inv_hardware ";
