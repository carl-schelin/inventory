...
# Description: 
  ...
  if (isset($_SESSION['username'])) {
    ...
    if (check_userlevel($AL_Edit)) {
      ...
      $rule_source      = return_Index($a_rules['rule_source'],      "select src_id  from source_node   where src_deleted = 0  order by src_node         ");
      $rule_application = return_Index($a_rules['rule_application'], "select app_id  from application   where app_deleted = 0  order by app_description  ");
