...
# Description: Retrieve data and update the database with the new info. Prepare and display the table
  ...
  if (isset($_SESSION['username'])) {
    ...
    if (check_userlevel($db, $AL_Edit)) {
      ...
      $output .= "<tr>\n";
      $output .= "  <th class=\"ui-state-default\">RSDP By Days Listing</th>\n";
      $output .= "  <th class=\"ui-state-default\" width=\"20\"><a href=\"javascript:;\" onmousedown=\"toggleDiv('days-listing-help');\">Help</a></th>\n";
      $output .= "</tr>\n";
      $output .= "</table>\n";

      $output .= "<div id=\"days-listing-help\" style=\"display: none\">\n";
      ...
      $output .= "<p>Clicking on an RSDP ID will bring up a single graph of that server based on number of days between tasks.</p>\n";

      $output .= "<p>Click <a href=\"timelines.days.csv.php?start=" . $formVars['start'] . "&end=" . $formVars['end'] . "&group=" . $formVars['group'] . "\" target=\"_blank\">here</a> to get an importable page of this data.</p>\n";
...
# divide the seconds by 60 to get minutes, then divide the minutes to get hours, then divide the hours by 24 to get the number of days.
        ...
        $output .= "<tr>\n";
        $output .= "<td class=\"" . $class . "\"><a href=\"timelines.graph.php?rsdp=" . $a_rsdp_server['rsdp_id'] . "\">" . $a_rsdp_server['rsdp_id'] . "</a></td>\n";
        for ($i = 1; $i < 19; $i++) {
          if ($diff[$i] > 0) {
      ...
      print "document.getElementById('days_mysql').innerHTML = '" . mysqli_real_escape_string($db, $output) . "';\n";
