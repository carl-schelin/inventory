...
function attach_file( p_script_url, complete ) {
  ...
  af_url += '&app_tested='     + af_form.app_tested.checked;
  af_url += '&app_integrated=' + af_form.app_integrated.checked;
  af_url += '&app_failover='   + af_form.app_failover.checked;
  af_url += '&app_concheck='   + af_form.app_concheck.checked;

  if (complete === 1) {
    var question  = "This Application form is ready to submit.\n\n";
...
function validate_Form() {
  ...
  if (document.rsdp.app_concheck.checked === false) {
    set_Class('app_concheck', 'ui-state-error');
    $( "#check-1" ).button( "option", "label", "Application Configuration checklist has NOT been completed." );
    vf_submit = 0;
  } else {
    set_Class('app_concheck', 'ui-widget-content');
    $( "#check-1" ).button( "option", "label", "Application Configuration checklist HAS been completed." );
  }

  if (document.rsdp.app_tested.checked === false) {
    set_Class('app_tested', 'ui-state-error');
    $( "#check-2" ).button( "option", "label", "Application has NOT been tested." );
    vf_submit = 0;
  } else {
    set_Class('app_tested', 'ui-widget-content');
    $( "#check-2" ).button( "option", "label", "Application HAS been tested." );
  }

  if (document.rsdp.app_integrated.checked === false) {
    set_Class('app_integrated', 'ui-state-error');
    $( "#check-3" ).button( "option", "label", "Integration testing is NOT complete." );
    vf_submit = 0;
  } else {
    set_Class('app_integrated', 'ui-widget-content');
    $( "#check-3" ).button( "option", "label", "Integration testing HAS been completed." );
  }

  if (document.rsdp.app_failover.checked === false) {
    set_Class('app_failover', 'ui-state-error');
    $( "#check-4" ).button( "option", "label", "Fail-Over testing is NOT complete." );
    vf_submit = 0;
  } else {
    set_Class('app_failover', 'ui-widget-content');
    $( "#check-4" ).button( "option", "label", "Fail-Over testing HAS been completed." );
  }
...
function clear_fields() {
  ...
  show_file('<?php print $RSDProot; ?>/application/configured.fill.php' + '?rsdp=<?php print $formVars['rsdp']; ?>');
}
...
<tr>
  <th class="ui-state-default">Application Configuration: <?php print $a_rsdp_osteam['os_sysname']; ?></th>
...
<h2>Application Configuration Page</h2>

<p>This task is typically performed by the team that will be installing the Application. There are five tabs providing Project and System information, 
team checklist, a list of items that must be checked off before the task can be completed, and a comments page for passing information to various teams.</p>
...
Once completed, the button is enabled. Clicking it identifies the task as completed. A ticket will be created for 
...
<?php print submit_RSDP( $formVars['rsdp'], 17, $RSDProot . "/application/configured.mysql.php", "rsdp_apppoc", "rsdp_application", 0); ?>
...
<table class="ui-styled-table">
<tr>
  <th class="ui-state-default" colspan="3">Application Configuration Checklist</th>
</tr>
<tr>
  <td id="app_concheck"  ><input type="checkbox" id="check-1" name="app_concheck"   onchange="validate_Form();"><label for="check-1"></label></td>
</tr>
<?php print return_Checklist( $formVars['rsdp'], 17); ?>
</table>
...
<tr>
  <td id="app_tested"    ><input type="checkbox" id="check-2" name="app_tested"     onchange="validate_Form();"><label for="check-2"></label></td>
</tr>
<tr>
  <td id="app_integrated"><input type="checkbox" id="check-3" name="app_integrated" onchange="validate_Form();"><label for="check-3"></label></td>
</tr>
<tr>
  <td id="app_failover"  ><input type="checkbox" id="check-4" name="app_failover"   onchange="validate_Form();"><label for="check-4"></label></td>
</tr>
</table>
