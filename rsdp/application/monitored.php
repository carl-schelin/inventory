...
function attach_file( p_script_url, complete ) {
  ...
  af_url += '&app_monitor='    + af_form.app_monitor.checked;
  af_url += '&app_verified='   + af_form.app_verified.checked;
  af_url += '&app_moncheck='   + af_form.app_moncheck.checked;
...
function validate_Form() {
  ...
  if (document.rsdp.app_moncheck.checked === false) {
    set_Class('app_moncheck', 'ui-state-error');
    $( "#check-1" ).button( "option", "label", "Application Monitoring checklist has NOT been completed." );
    vf_submit = 0;
  } else {
    set_Class('app_moncheck', 'ui-widget-content');
    $( "#check-1" ).button( "option", "label", "Application Monitoring checklist HAS been completed." );
  }

  if (document.rsdp.app_monitor.checked == false) {
    set_Class('app_monitor', 'ui-state-error');
    $( "#check-2" ).button( "option", "label", "Application Monitoring has NOT been configured." );
    vf_submit = 0;
  } else {
    set_Class('app_monitor', 'ui-widget-content');
    $( "#check-2" ).button( "option", "label", "Application Monitoring HAS been configured." );
  }

  if (document.rsdp.app_verified.checked == false) {
    set_Class('app_verified', 'ui-state-error');
    $( "#check-3" ).button( "option", "label", "Application Monitoring has NOT been verified." );
    vf_submit = 0;
  } else {
    set_Class('app_verified', 'ui-widget-content');
    $( "#check-3" ).button( "option", "label", "Application Monitoring HAS been verified." );
  }
...
function clear_fields() {
  ...
  show_file('<?php print $RSDProot; ?>/application/monitored.fill.php' + '?rsdp=<?php print $formVars['rsdp']; ?>');
}
...
<tr>
  <th class="ui-state-default">Application Monitoring: <?php print $a_rsdp_osteam['os_sysname']; ?></th>
...
<h2>Application Monitored Page</h2>

<p>This task is typically performed by the Monitoring Team in conjunction with the Applications Team to 
ensure the installed Application is being monitored and testing has been completed. There are five tabs 
providing Project and System information, team specific checklist, a list of items that must be checked 
off before the task can be completed, and a comments page for passing information to various teams.</p>
...
a ticket will be created for the next task and an email will be sent notifying the responsible team or 
individual that their task is ready to be worked.</li>
  </ul></li>
</ul>
...
<?php print submit_RSDP( $formVars['rsdp'], 16, $RSDProot . "/application/monitored.mysql.php", "rsdp_monitorpoc", "", $GRP_Monitoring); ?>
...
<tr>
  <td id="app_monitor" ><input type="checkbox" id="check-2" name="app_monitor"  onchange="validate_Form();"><label for="check-2"></label></td>
</tr>
<tr>
  <td id="app_verified"><input type="checkbox" id="check-3" name="app_verified" onchange="validate_Form();"><label for="check-3"></label></td>
</tr>
</table>
