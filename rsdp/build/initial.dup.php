...
######
  ...
  mysql_query($q_server) or die($q_server . ": " . mysql_error());
...
######
  ...
  mysql_query($q_backup) or die($q_backup . ": " . mysql_error());
...
######
    ...
    mysql_query($q_comments) or die($q_comments . ": " . mysql_error());
  }
...
######
      ...
      mysql_query($q_filesystem) or die($q_filesystem . ": " . mysql_error());
    }
  }
...
######
  if (mysql_num_rows($q_rsdp_osteam) > 0) {
    ...
    mysql_query($q_osteam) or die($q_osteam . ": " . mysql_error());
  } else {
######
...
# note, this is all physical server details for the Data Center folks
    ...
    mysql_query($q_platform) or die($q_platform . ": " . mysql_error());
  } else {
######
...
######
      ...
      mysql_query($q_interface) or die($q_interface . ": " . mysql_error());
    }
  }
...
#   if no, create a new 'copy to' record
    ...
    if (mysql_num_rows($q_rsdp_infrastructure) > 0) {
      ...
      if (mysql_num_rows($q_infrastructure) > 0) {
        $a_infrastructure = mysql_fetch_array($q_infrastructure);
...
######
  if ($formVars['chk_app1']) {
    ...
    if (mysql_num_rows($q_applications) > 0) {
      $a_applications = mysql_fetch_array($q_applications);
