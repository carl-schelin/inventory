...
#0 16 * * * /usr/local/bin/php /var/www/html/inventory/scripts/server.exclude.php > /usr/local/admin/install/unixsuite/etc/messages.exclude
  ...
  while ($a_inv_excludes = mysqli_fetch_array($q_inv_excludes)) {
    ...
    print $a_inv_excludes['inv_name'] . ".*" . $a_inv_excludes['ex_text'] . "\n";
